# [GLOB-0004] エントリー・スナップショットのデータ契約 v2（rsi/hv 軸を追加）
...
| **Status** | `Accepted` |
| **Date** | 2026-06-04 |
| **Supersedes** | `GLOB-0003` |
| **Superseded by** | — |

## ❓ コンテキスト（背景と課題）

`GLOB-0003` でエントリー・スナップショットのデータ契約（凹みの深さ・出来高急増度・トレンド位置の3軸）を
定めた。運用の中で、客観での「答え合わせ」をより多面的に行うため、**RSI（売られすぎ度）と
年率ヒストリカル・ボラティリティ（HV）**を分析軸として追加したい。いずれも終値ベースで計算でき、
既存パイプラインに軽く足せる。スキーマ（`rows` の列）が変わるため、`GLOB-0003` を `Superseded` にして
本 ADR で契約を更新する。

## 💡 決定事項（Decision）

`GLOB-0003` の方針（監視リスト方式・Python事前計算→銘柄別JSON遅延取得・VolDipSignals同一定義）を
...
4. その他（監視リスト・遅延取得・派生キャッシュ・「データなし」の劣化動作）は `GLOB-0003` を踏襲する。
...
- 前身: `docs/adr/GLOB-0003-tradebook-entry-snapshot-data-contract.md`
