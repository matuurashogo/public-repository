...
test("withMatsuiFees: 不正な price/quantity の取引が同日他取引の手数料を汚染しない", () => {
  const trades = [
    { id: "1", date: "2026-03-01", code: "7203", side: "買", quantity: 100, price: 7000 }, // 70万
    { id: "2", date: "2026-03-01", code: "6758", side: "買", quantity: "", price: 5000 }, // 数量空欄=不正
  ];
  const out = withMatsuiFees(trades);
  // 不正取引は0扱い→当日合計は70万のみ→ボックスレート1100。NaNが伝播しない。
  assert.ok(Number.isFinite(out[0].fee));
  assert.equal(out[0].fee, 1100);
  assert.equal(out[1].fee, 0);
});

test("calcRealized: 数量/単価が不正な取引はスキップし保有を汚染しない（警告に記録）", () => {
  const trades = [
    { id: "1", date: "2026-03-01", code: "7203", side: "買", quantity: 100, price: 2000 },
    { id: "x", date: "2026-03-02", code: "7203", side: "買", quantity: NaN, price: 3000 }, // 不正
    { id: "2", date: "2026-03-10", code: "7203", side: "売", quantity: 100, price: 2500 },
  ];
  const { records, holdings, warnings } = calcRealized(trades);
  // 不正な買いはスキップ→平均取得単価は2000のまま、保有は0
  assert.equal(records[0].avgCost, 2000);
  assert.equal(records[0].pnl, 50000); // (2500-2000)*100
  assert.equal(holdings["7203"].quantity, 0);
  assert.ok(warnings.some((w) => w.includes("不正")));
});

test("histogramBins: 空入力は空配列", () => {
  assert.deepEqual(histogramBins([]), []);
  assert.deepEqual(histogramBins(null), []);
});

test("histogramBins: 件数が保存され、ビン数ぶん返る", () => {
  const pnls = [-50000, 30000, 30000];
  const bins = histogramBins(pnls, 7);
  assert.equal(bins.length, 7);
  // すべての件数の合計は入力件数と一致する
  assert.equal(bins.reduce((s, b) => s + b.count, 0), 3);
  // span=50000, width=100000/7。-50000 は先頭ビン、+30000 は同じビンに2件
  assert.equal(bins[0].count, 1); // 最大の損失は左端
  const gainBin = bins.find((b) => b.count === 2);
  assert.ok(gainBin && gainBin.sign === "gain"); // 利益側に2件まとまる
});

test("histogramBins: 損は loss / 益は gain で符号付け", () => {
  const bins = histogramBins([-10000, 20000], 7);
  for (const b of bins) {
    assert.equal(b.sign, b.mid < 0 ? "loss" : "gain");
  }
  // 左端は必ず損、右端は必ず益
  assert.equal(bins[0].sign, "loss");
  assert.equal(bins[bins.length - 1].sign, "gain");
});

test("histogramBins: 範囲外の値もクランプして取りこぼさない", () => {
  // すべて同値でも全件どこかのビンに入る（idxのクランプ確認）
  const bins = histogramBins([10000, 10000], 7);
  assert.equal(bins.reduce((s, b) => s + b.count, 0), 2);
});
