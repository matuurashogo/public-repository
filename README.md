...
- 取引の手入力（約定日 / 銘柄コード / 売買 / 数量 / 約定単価）。銘柄コードから銘柄名を自動表示。
- 平均法による実現損益、**概算税額（20.315%）・税引後損益**の自動表示。
- 年間 / 月間 / 銘柄別の集計表と、**累積損益の折れ線グラフ**（Chart.js 同梱・オフライン可）。
...
CSVインポート/エクスポート、手数料の取引別入力、評価損益（含み損益）・現在株価、配当、信用の空売り。
